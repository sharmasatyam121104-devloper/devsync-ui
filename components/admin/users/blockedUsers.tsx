...
const BlockedUsers = () => {
  ...
  const { data, error, isLoading } = useSWR(
    `/admin/blocked-users?page=${page}&limit=${limit}`,
    fetcher,
    {
  ...
  const users: User[] = data?.blockedUsers || []
  const total = data?.totalBlockeduser || 0
    ...
    <div className="lg:p-4">
      ...
      {isLoading ? (
        ...
        <div className="py-20 flex justify-center">
          <Empty description="No Blocked Users Found" />
        </div>
              ...
              <Card key={user._id} className="rounded-2xl shadow-sm hover:shadow-md transition">
                <CardContent className="p-4 space-y-3">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    {/* 🔴 Red badge */}
                    <span className="text-xs px-2 py-1 rounded-full bg-red-100 text-red-600 uppercase font-medium">
                      {user.status}
                    </span>
                  </div>
